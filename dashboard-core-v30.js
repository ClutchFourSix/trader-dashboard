(function(){
  const SECTION_LAYOUT_KEY='td_layout_v30_sections';
  const TILE_LAYOUT_KEYS={commands:'td_layout_v30_commands',clocks:'td_layout_v30_clocks',assets:'td_layout_v30_assets'};
  const SECTION_SIZE_KEY='td_layout_v30_sizes';
  ...
  function isWeekendNow(){ const d=new Date(); const day=d.getDay(); return day===0 || day===6; }
  ...
  function normalizeNumbers(){
    qsa('#assetGrid .asset-card .metric-value, #assetGrid .asset-card .metric .value, #assetGrid .asset-card [data-role="value"]').forEach(el=>{
      el.textContent = String(el.textContent||'').replace(/\s+/g,' ');
    });
  }
  ...
  function computeScore(s){
    ...
    if(ageMin <= 30) story += 14; else if(ageMin <= 120) story += 9; else if(ageMin <= 360) story += 4; else if(ageMin <= 1440) story += 1; else if(ageMin <= 2880) story -= 4; else story -= 10;
    story += Math.min(12,s.relevanceScore||0); if(s.red) story += 15;
  ...
  function buildSidePanel(counts,realCount){const sources=getSourceScores(); const leaderboard=Object.entries(sources).sort((a,b)=>b[1].score-a[1].score).map(([k,s])=>`<tr><td>${esc(s.name)}</td><td>${s.score}</td><td>${counts[k]||0}</td></tr>`).join(''); return `<div class="side-stack"><div class="glass-sm section"><div class="section-title">Source Scores</div><table class="table"><thead><tr><th>Source</th><th>Score</th><th>Shown</th></tr></thead><tbody>${leaderboard}</tbody></table></div><div class="glass-sm section"><div class="section-title">Feed Rules</div><div class="subtle">Best real articles first, recency second. Showing ${realCount} real stories.</div></div></div>`;}
  function renderNewsQualityFirst(){const root=qs('#newsIntelRoot'); if(!root) return; const live=getCoreNews(); const filtered=live.filter(isRealArticle).map(computeScore).sort((a,b)=> b.final - a.final || a.ageMin - b.ageMin); const pickedData=dedupeAndLimit(filtered,2,10); const picked=pickedData.items; const counts=pickedData.counts; if(!picked.length){root.innerHTML=`<div class="news-layout"><div class="story-stack"><div class="glass-sm section"><div class="section-title">No Real Article Headlines Available</div><div class="subtle">The current feeds did not return enough genuine article headlines. No placeholder site cards were inserted.</div></div></div>${buildSidePanel(counts,0)}</div>`; return;} root.innerHTML=`<div class="news-layout"><div class="story-stack">${picked.map((s,idx)=>`<div class="story ${idx===0?'top':''}"><div class="story-rank">#${idx+1} • ${esc(getSourceScores()[s.source]?.name||s.source)} • ${s.ageMin<60?`${s.ageMin}m ago`:`${Math.floor(s.ageMin/60)}h ${s.ageMin%60}m ago`}</div><div class="story-title"><a href="${esc(s.link)}" target="_blank">${esc(s.title)}</a></div><div class="story-summary">${esc(s.summary||'')}</div><div class="story-score ${s.final>=80?'high':s.final>=60?'mid':'low'}">Overall: ${s.final} | Source: ${s.src} | Story: ${s.story}</div></div>`).join('')}</div>${buildSidePanel(counts,picked.length)}</div>`;}
  ...
  window.addEventListener('load',()=>{markZones(); applySavedOrder('[data-layout-zone="main"]','.draggable-section','data-layout-id',SECTION_LAYOUT_KEY,['news','red','assets','charts','add','provider']); wireSectionDrag(); initResizables(); setTimeout(()=>{ensureTileHandles(); applySavedOrder('[data-tile-zone="commands"]','.draggable-tile','data-tile-id',TILE_LAYOUT_KEYS.commands,['commands-0','commands-1','commands-2','commands-3','commands-4']); applySavedOrder('[data-tile-zone="clocks"]','.draggable-tile','data-tile-id',TILE_LAYOUT_KEYS.clocks,['clocks-0','clocks-1','clocks-2','clocks-3','clocks-4','clocks-5']); applySavedOrder('[data-tile-zone="assets"]','.draggable-tile','data-tile-id',TILE_LAYOUT_KEYS.assets,[]); wireTileDrag(); normalizeNumbers(); patchWeekendClocks();},1000); setTimeout(()=>{renderNewsQualityFirst(); patchWeekendClocks(); normalizeNumbers();},1800); setTimeout(()=>{renderNewsQualityFirst(); patchWeekendClocks(); normalizeNumbers();},3600); const refresh=qs('#refreshBtn'); if(refresh) refresh.addEventListener('click',()=>setTimeout(()=>{renderNewsQualityFirst(); patchWeekendClocks(); normalizeNumbers();},1000));});
})();
