(function(){
  const SECTION_LAYOUT_KEY='td_layout_v24_sections';
  const TILE_LAYOUT_KEYS={commands:'td_layout_v24_commands',clocks:'td_layout_v24_clocks',assets:'td_layout_v24_assets'};
  const SECTION_SIZE_KEY='td_layout_v24_sizes';
  ...
  function applySavedSectionSizes(){
    ...
    qsa('.resizable-section').forEach(el=>{const meta=sizes[el.dataset.layoutId]; if(!meta) return; if(meta.height) el.style.minHeight=meta.height; if(meta.size) el.dataset.size=meta.size;});
  }
  function saveSectionSizes(){
    const sizes={}; qsa('.resizable-section').forEach(el=>{sizes[el.dataset.layoutId]={height:el.style.minHeight||'',size:el.dataset.size||'md'}}); saveJson(SECTION_SIZE_KEY,sizes);
  }
  ...
  function renderNewsQualityFirst(){
    ...
    ensureTileHandles(); wireTileDrag();
  }
