(function(){
  ...
  function buildSidePanel(counts,realCount){
    ...
    return `<div class="side-stack"><div class="glass-sm section"><div class="section-title">Source Scores</div><table class="table"><thead><tr><th>Source</th><th>Score</th><th>Shown</th></tr></thead><tbody>${leaderboard}</tbody></table></div><div class="glass-sm section"><div class="section-title">Feed Rules</div><div class="subtle">v10-style board. Real article headlines only. Public/open links only. ${realCount<10?`Showing ${realCount} real stories right now.`:'Showing up to 10 real stories.'}</div></div><div class="glass-sm section"><div class="section-title">Accurate Social Accounts</div><div class="list">${SOCIAL.map(a=>`<div class="row"><div><div class="row-title">${esc(a.name)}</div><div class="row-meta">${esc(a.focus)} • score ${a.score}</div></div><div class="provider-badge-row">${a.links.map(([label,url])=>`<a class="story-link-chip" href="${esc(url)}" target="_blank">${esc(label)}</a>`).join('')}</div></div>`).join('')}</div></div></div>`;
  }
  function renderStableNews(){
    ...
    const signature=JSON.stringify({items:picked.map(x=>[x.title,x.link,x.final]),counts});
  ...
  function scheduleNewsRenders(){
    setTimeout(renderStableNews, 700);
    setTimeout(renderStableNews, 1800);
    setTimeout(renderStableNews, 3200);
    const refresh=document.getElementById('refreshBtn');
    if(refresh) refresh.addEventListener('click',()=>setTimeout(renderStableNews,900));
  }
  window.addEventListener('load', scheduleNewsRenders);
})();
