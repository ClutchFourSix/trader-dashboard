(function(){
  ...
  function saveSectionSizes(){
    const sizes={};
    qsa('.resizable-section').forEach(el=>{ sizes[el.dataset.layoutId]={height:el.style.minHeight||'',size:el.dataset.size||'md'}; });
  ...
  function closestByY(container,y){
    ...
    return els.reduce((closest,el)=>{const box=el.getBoundingClientRect();const delta=Math.abs(y-(box.top+box.height/2));return delta<closest.delta?{delta,el}:closest},{delta:Infinity,el:null}).el;
  }
  function startPointerDrag(section,ev){
    ...
    function onMove(e){
      ...
      if(target&&target!==draggingEl&&target!==placeholder){const box=target.getBoundingClientRect(); if(e.clientY<box.top+box.height/2) main.insertBefore(placeholder,target); else main.insertBefore(placeholder,target.nextSibling);}
    }
    function onUp(){document.removeEventListener('pointermove',onMove);document.removeEventListener('pointerup',onUp);if(ghost) ghost.remove(); ghost=null;draggingEl.style.display=''; draggingEl.classList.remove('dragging-hidden'); placeholder.replaceWith(draggingEl); draggingEl=null; placeholder=null; saveJson(SECTION_LAYOUT_KEY,currentSectionOrder());}
    document.addEventListener('pointermove',onMove); document.addEventListener('pointerup',onUp,{once:true});
  }
  function wireSectionDrag(){qsa('.draggable-section .section-head-inline').forEach(head=>{head.onpointerdown=(e)=>{const sec=head.closest('.draggable-section'); if(sec) startPointerDrag(sec,e)}})}
  ...
  function buildSidePanel(counts,realCount){
    ...
    return `<div class="side-stack"><div class="glass-sm section"><div class="section-title">Source Scores</div><table class="table"><thead><tr><th>Source</th><th>Score</th><th>Shown</th></tr></thead><tbody>${leaderboard}</tbody></table></div><div class="glass-sm section"><div class="section-title">Feed Rules</div><div class="subtle">Best real articles first, with recency as a secondary factor. Newer stories still get a boost, but slightly older high-quality stories can rank above weaker fresh ones. Showing ${realCount} real stories.</div></div></div>`;
  }
  function renderNewsQualityFirst(){
    ...
    root.innerHTML=`<div class="news-layout"><div class="story-stack">${picked.map((s,idx)=>`<div class="story ${idx===0?'top':''}"><div class="story-rank">#${idx+1} • ${esc((window.V11_NEWS_SOURCES||{})[s.source]?.name||s.source)} • ${s.ageMin<60?`${s.ageMin}m ago`:`${Math.floor(s.ageMin/60)}h ${s.ageMin%60}m ago`}</div><div class="story-title"><a href="${esc(s.link)}" target="_blank">${esc(s.title)}</a></div><div class="story-summary">${esc(s.summary||'')}</div><div class="story-score ${s.final>=80?'high':s.final>=60?'mid':'low'}">Overall: ${s.final} | Source: ${s.src} | Story: ${s.story}</div></div>`).join('')}</div>${buildSidePanel(counts,picked.length)}</div>`;
  }
  ...
  window.addEventListener('load',()=>{
    ...
    setTimeout(renderNewsQualityFirst,700);
    setTimeout(renderNewsQualityFirst,1800);
    const refresh=qs('#refreshBtn'); if(refresh) refresh.addEventListener('click',()=>setTimeout(renderNewsQualityFirst,900));
  });
})();
